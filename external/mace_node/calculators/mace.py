        ...
        if len(model_paths) > 1:
            ...
            if model_type in ["MACE", "EnergyDipoleMACE"]:
                self.implemented_properties.extend(
                    ["energies", "energy_var", "node_energy_var","forces_comm", "stress_var","forcenormstd"]
                )
            elif model_type == "DipoleMACE":
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        if self.model_type in ["MACE", "EnergyDipoleMACE"]:
            ...
            if self.num_models > 1:
                ...
                self.results["node_energy_var"] = (
                    torch.var(ret_tensors["node_energy"], dim=0, unbiased=False)
                    .cpu()
                    .numpy()
                    * self.energy_units_to_eV
                )
                ...
                # atomic energy uncertainty
                if self.mode>0:
                   self.Eave = self.results["energy"]
                   self.results["Eave"] = self.Eave * self.energy_units_to_eV
                   self.Fave = self.results["forces"].copy()
                   varnorm = self.results["energy_var"]/len(self.Fave)
                   force_on_var = np.dot(self.results["energies"] - self.Eave,(self.results["forces_comm"]-self.Fave).reshape((self.num_models,3*len(self.Fave)))).reshape(self.Fave.shape)*2./(self.num_models*len(self.Fave))

                   if self.mode==1:
                      exp = self.bias_amplitude * np.exp(-varnorm / self.bias_width) * len(self.Fave)
                      self.results['energy'] += exp #bias Energy
                      self.results['forces'] -= exp * force_on_var / self.bias_width

                # atomic force uncertainty
                fdiff = ret_tensors["forces"] - torch.mean(ret_tensors["forces"], dim=0)  # Shape: [num_models, num_atoms, 3]
                norms = torch.norm(fdiff, dim=2)  # Shape: [num_models, num_atoms]
                squared_norms = norms ** 2  # Shape: [num_models, num_atoms]
                mean_squared_norms = torch.mean(squared_norms, dim=0)  # Shape: [num_atoms]
                self.results['forcenormstd'] = torch.sqrt(mean_squared_norms)  # Shape: [num_atoms]
                self.results['forcenormstd'] = (
                    self.results['forcenormstd'].cpu().numpy()
                    * self.energy_units_to_eV
                    / self.length_units_to_A
                )
